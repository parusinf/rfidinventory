...
struct NUR_DEVICECAPS
{
	...
	WORD curCfgMaxAnt;	/**< Number of maximum possible antennas with current configuration. */
	WORD curCfgMaxGPIO;	/**< Number of maximum possible GPIO pins with current configuration. */
	WORD chipVersion;	/**< RFID chip version. (enum NUR_CHIPVER) */
	WORD moduleType;	/**< Module tpye. (enum NUR_MODULETYPE) */
	DWORD moduleConfigFlags; /**< Internal module config flags */

	BYTE res[SZ_NUR_DEVCAPS - 4*sizeof(DWORD) - 2*sizeof(int) - 7*sizeof(WORD)];	/**< The size left over of 128 bytes. */
};
...
{
	/** NUR module link frequency in Hz.
	 *
	...
	/** NUR module transmission level attenuation.
	 *
	...
	/** Zero based index of selected antenna. Value -1 - 3, where -1 means auto switch between antennas. 
	 * NOTE: Antenna must be enabled with antennaMask before set.
	 */
	int selectedAntenna;
	...
	/** Desired EPC Length filter in bytes. All other EPC's will be discarded. Use 255 to disable EPC length filter. */
	...
	/** Automode periodic inventory setup. */
	int periodSetup;
};
...
struct NUR_TAG_DATA
{
	...
	char rssi;			    /**< Tag RSSI; Only valid when meta data is fetched. */
...
struct NUR_INVEX_FILTER
{
	...
	/** Target session or SL. Indicated whether G2 select command modifies tag's SL flag or inventoried (A/B) flag. @sa enum NUR_INVENTORY_SESSION */
	BYTE target;
...
struct NUR_EPCENUMINFO
{
	...
	BOOL bReset;		/**< If nono-zero then the current TID information is cleared */	
};
 ...
 *
 * @sa NUR_CUSTOMHOP_PARAMS, NurApiGetCustomHoptable()
 * 
 * @param	hApi		Handle to valid NurApi object instance.
 ...
 * @param pPrimary			Pointer to the primary version string (in the loader the loader version and in the application the application version).
 * @param pSecondary		Pointer to the secondary version string (in the loader the application version and in the application the loader version).
 *
 ...
 *  
 * \section prelim Preliminary
 * This is preliminary documentation as is subject to change
 */
